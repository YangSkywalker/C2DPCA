...
data_path = '.\data\Coil20_32X32.mat';      % data path
...
%             end
%             end_time = clock;
%             fprintf('This is the %d-th iteration of C2DPCA on Coil20_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));
%         end
%         ACC_C2DPCA = setfield(ACC_C2DPCA, strcat('p',erase(num2str(pi),'.'),'_','c',erase(num2str(ci),'.')),ACC_temp);
%     end
% end
% save('.\result\Coil20_Original_hyperparameter','ACC_C2DPCA');
...
%         end
%         end_time = clock;
%         fprintf('This is the %d-th iteration of Lp-2-DPCA on Coil20_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));
%     end
%     ACC_Lp2DPCA = setfield(ACC_Lp2DPCA, strcat('p',erase(num2str(pi),'.')),ACC_temp);
% end
% save('.\result\Coil20_Original_hyperparameter','ACC_Lp2DPCA','-append');
...
%             end
%             end_time = clock;
%             fprintf('This is the %d-th iteration of G2DPCA on Coil20_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));
%         end
%         ACC_G2DPCA = setfield(ACC_G2DPCA, strcat('s',erase(num2str(si),'.'),'_','p',erase(num2str(pi),'.')),ACC_temp);
%     end
% end
% save('.\result\Coil20_Original_hyperparameter','ACC_G2DPCA','-append');
% 
...
%             end
%             end_time = clock;
%             fprintf('This is the %d-th iteration of FCPCA on Coil20_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));
%         end
%         ACC_FCPCA = setfield(ACC_FCPCA, strcat('p',erase(num2str(pi),'.'),'_','c',erase(num2str(ci),'.')),ACC_temp);
%     end
% end
% save('.\result\Coil20_Original_hyperparameter','ACC_FCPCA','-append');
% 
...
%         end
%         end_time = clock;
%         fprintf('This is the %d-th iteration of Capped 2DPCA-OM on Coil20_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));
%     end
%     ACC_Capped2DPCAOM = setfield(ACC_Capped2DPCAOM, strcat('epsilon',erase(num2str(epsiloni),'.')),ACC_temp);
% end
% save('.\result\Coil20_Original_hyperparameter','ACC_Capped2DPCAOM','-append');
...
for si = s
    for pi = p
        ...
        for i = 1:iterations
            ...
            end_time = clock;
            fprintf('This is the %d-th iteration of 2DPCA-2-LP on Coil20_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));
        end
        ACC_D2PCA2Lp = setfield(ACC_D2PCA2Lp, strcat('s',erase(num2str(si),'.'),'_','p',erase(num2str(pi),'.')),ACC_temp);
    end
end
save('.\result\Coil20_Original_hyperparameter','ACC_D2PCA2Lp','space','-append');
