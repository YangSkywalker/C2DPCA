...
data_path = '.\data\Jaffe_32X32.mat';      % data path
...
for pi = p
    for ci = c
        ...
        for i = 1:iterations
            ...
            end_time = clock;
            fprintf('This is the %d-th iteration of C2DPCA on Jaffe_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));
        end
        ACC_C2DPCA = setfield(ACC_C2DPCA, strcat('p',erase(num2str(pi),'.'),'_','c',erase(num2str(ci),'.')),ACC_temp);
    end
end
save('.\result\Jaffe_Original','ACC_C2DPCA');
...
for pi = p
    ...
    for i = 1:iterations
        ...
        end_time = clock;
        fprintf('This is the %d-th iteration of Lp-2-DPCA on Jaffe_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));
    end
    ACC_Lp2DPCA = setfield(ACC_Lp2DPCA, strcat('p',erase(num2str(pi),'.')),ACC_temp);
end
save('.\result\Jaffe_Original','ACC_Lp2DPCA','-append');
...
for si = s
    for pi = p
        ...
        for i = 1:iterations
            ...
            end_time = clock;
            fprintf('This is the %d-th iteration of G2DPCA on Jaffe_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));
        end
        ACC_G2DPCA = setfield(ACC_G2DPCA, strcat('s',erase(num2str(si),'.'),'_','p',erase(num2str(pi),'.')),ACC_temp);
    end
end
save('.\result\Jaffe_Original','ACC_G2DPCA','-append');
...
for pi = p
    for ci = c
        ...
        for i = 1:iterations
            ...
            end_time = clock;
            fprintf('This is the %d-th iteration of FCPCA on Jaffe_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));
        end
        ACC_FCPCA = setfield(ACC_FCPCA, strcat('p',erase(num2str(pi),'.'),'_','c',erase(num2str(ci),'.')),ACC_temp);
    end
end
save('.\result\Jaffe_Original','ACC_FCPCA','-append');
...
for epsiloni = epsilon
    ...
    for i = 1:iterations
        ...
        end_time = clock;
        fprintf('This is the %d-th iteration of Capped 2DPCA-OM on Jaffe_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));
    end
    ACC_Capped2DPCAOM = setfield(ACC_Capped2DPCAOM, strcat('epsilon',erase(num2str(epsiloni),'.')),ACC_temp);
end
save('.\result\Jaffe_Original','ACC_Capped2DPCAOM','-append');
...
for si = s
    for pi = p
        ...
        for i = 1:iterations
            ...
            end_time = clock;
            fprintf('This is the %d-th iteration of 2DPCA-2-LP on Jaffe_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));
        end
        ACC_D2PCA2Lp = setfield(ACC_D2PCA2Lp, strcat('s',erase(num2str(si),'.'),'_','p',erase(num2str(pi),'.')),ACC_temp);
    end
end
save('.\result\Jaffe_Original','ACC_D2PCA2Lp','space','-append');
