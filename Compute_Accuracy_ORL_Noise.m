...
% store classification accuracy of C2DPCA (p=1, c=0.5)
...
% store classification accuracy of Lp-2-DPCA (p = 1.2)
ACC_Lp2DPCA = zeros(iterations,pcomp);
% store classification accuracy of G2DPCA (s = 1.2, p = 0.5)
ACC_G2DPCA = zeros(iterations,pcomp);
% store classification accuracy of 2DPCA-2-Lp (s = 1.2, p = 0.5)
ACC_D2PCA2Lp = zeros(iterations,pcomp);
% store classification accuracy of FCPCA (p = 1, s = 0.5)
...
for i = 1:iterations
    ...
    % compute principal components of C2DPCA (p=1.2, c=10)
    start_time = clock;
    W_CL2p = C2DPCA(x_train, maxDim, 1.2, 10);
    % acc
    for j = space
    ...
    end_time = clock;
    fprintf('This is the %d-th iteration of C2DPCA (p=1.2, c=10) on ORL_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));
    ...
    % compute principal components of Capped 2DPCA-OM (k1 = 30, k2= 30, epislon = 5)
    start_time = clock;
    W_Capped2DPCAOM = Capped2DPCAOM(x_train, maxDim, maxDim, 5);
    % acc
    for j = space
    ...
    end_time = clock;
    fprintf('This is the %d-th iteration of Capped 2DPCA-OM (epsilon = 5) on ORL_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));

    % compute principal components of Lp-2-DPCA (p = 1.6)
    start_time = clock;
    W_Lp2DPCA = Lp2DPCA(x_train, maxDim, 1.6); 
    % acc
    for j = space
    ...
    end_time = clock;
    fprintf('This is the %d-th iteration of Lp-2-DPCA (p = 1.6) on ORL_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));

    % compute principal components of G2DPCA (s = 1.2, p = 1.5)
    start_time = clock;
    W_G2DPCA = G2DPCA(x_train, 1.2, 1.5, maxDim);
    % acc
    for j = space
    ...
    end_time = clock;
    fprintf('This is the %d-th iteration of G2DPCA (s = 1.2, p = 1.5) on ORL_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));
    ...
    % compute principal components of FCPCA (p = 1.2, c = 10)
    start_time = clock;
    ...
    %%% FCPCA
    W_FCPCA = FCPCA(x_train_vec, maxDim, 1.2, 10);
    % acc
    for j = space
    ...
    end_time = clock;
    fprintf('This is the %d-th iteration of FCPCA (p = 1.2, c = 10) on ORL_32X32, the elapsed time is %f s \n',i,etime(end_time,start_time));

end

save('.\result\ORL_16X16_1','ACC_C2DPCA', 'ACC_FCPCA', 'ACC_Capped2DPCAOM','ACC_D2PCA2Lp', 'ACC_Lp2DPCA', 'ACC_G2DPCA', 'ACC_D2PCA', 'space');
